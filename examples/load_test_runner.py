...
class LoadTestRunner:
    ...
    def run_sequential_test(self) -> Dict[str, Any]:
        ...
        metrics["success_rate_percent"] = (
            metrics["success_count"] / self.num_interactions
        ) * 100
        ...
        metrics["latency_stats"] = {
            ...
            "stdev_ms": (
                statistics.stdev(metrics["action_times_ms"])
                if len(metrics["action_times_ms"]) > 1
                else 0
            ),
    ...
    def run_concurrent_test(self) -> Dict[str, Any]:
        ...
        with ThreadPoolExecutor(max_workers=self.num_workers) as executor:
            futures = {
                executor.submit(worker_thread, i): i for i in range(self.num_workers)
            }
    ...
    def _print_test_summary(self, test_name: str, metrics: Dict[str, Any]):
        ...
        print("=" * 70)
        print(
            f"Total Actions:              {metrics.get('total_actions', metrics.get('total_receipts', 0)):,}"
        )
        ...
        print(
            f"Throughput:                 {metrics['throughput_actions_per_sec']:.0f} actions/sec"
        )
...
def main():
    ...
    runner = LoadTestRunner(
        num_interactions=args.interactions, num_workers=args.workers
    )

...
class LoadTestConfig:
    ...
    def from_environment(cls) -> Dict[str, Any]:
        ...
        if "CIAF_LOAD_TEST_SCALE" in os.environ:
            ...
            print(
                f"📊 Load test scale: {scale_str} ({config['scale_actions']:,} actions)"
            )
        ...
        if "CIAF_LOAD_TEST_DURATION_HOURS" in os.environ:
            config["max_duration_hours"] = float(
                os.environ["CIAF_LOAD_TEST_DURATION_HOURS"]
            )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        print(
            "Or set environment variables: CIAF_LOAD_TEST_SCALE, CIAF_LOAD_TEST_WORKERS, etc."
        )

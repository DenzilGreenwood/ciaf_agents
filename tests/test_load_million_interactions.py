...
class LoadTestMetrics:
    ...
    def print_summary(self):
        ...
        print(
            f"  Successful:              {self.successful_actions:,} ({self.success_rate:.2f}%)"
        )
        print(f"  Failed:                  {self.failed_actions:,}")
        print(
            f"  Total Duration:          {self.total_duration_seconds:.2f}s ({self.total_duration_seconds/3600:.2f}h)"
        )

        print(f"\nThroughput Analysis:")
        print(
            f"  Average Throughput:      {self.throughput_actions_per_second:,.0f} actions/sec"
        )
        print(
            f"  Peak Throughput:         {max([t[1] for t in self.throughput_samples] if self.throughput_samples else [0]):,.0f} actions/sec"
        )
        ...
        if self.gc_pauses:
            ...
            print(
                f"  Avg Pause:               {statistics.mean(self.gc_pauses)*1000:.2f}ms"
            )
...
def execute_action_scenario(
    executor, agent: Identity, action_num: int
) -> Tuple[bool, float]:
...
class TestMillionAgentInteractions:
    ...
    def test_sequential_million_interactions(self, setup_infrastructure):
        ...
        # Assertions
        assert metrics.successful_actions >= (
            num_actions * 0.95
        ), f"Success rate below 95%: {metrics.success_rate:.1f}%"
        assert (
            metrics.throughput_actions_per_second >= 1000
    ...
    def test_concurrent_agent_interactions(self, setup_infrastructure):
                    ...
                    with completed_lock:
                        ...
                        if completed_actions - last_checkpoint >= checkpoint_interval:
                            ...
                            metrics.memory_samples.append(
                                (completed_actions, psutil.Process().memory_info().rss)
                            )
                            metrics.throughput_samples.append(
                                (completed_actions, checkpoint_throughput)
                            )

                            print(
                                f"  Checkpoint {checkpoint_num}: {completed_actions/1e9:.2f}B actions | "
        ...
        with ThreadPoolExecutor(max_workers=num_workers) as executor_pool:
            futures = [
                executor_pool.submit(worker_thread, i) for i in range(num_workers)
            ]
        ...
        if metrics.memory_samples:
            ...
            mem_growth_mb = (
                metrics.memory_samples[-1][1] - metrics.memory_samples[0][1]
            ) / 1e6
            ...
            if metrics.throughput_samples:
                ...
                degradation = (
                    (1.0 - (last_throughput / first_throughput)) * 100
                ...
                if degradation > 10:
                    print(
                        f"    ⚠ Warning: Significant performance degradation detected"
                    )
        ...
        assert (
            metrics.throughput_actions_per_second > 0
    ...
    def test_evidence_vault_scaling(self, setup_infrastructure):
        ...
        print(
            f"Estimated Storage Size: {estimated_size:.1f} MB (avg 650 bytes/receipt)"
        )
    ...
    def test_multi_tenant_isolation_at_scale(self, setup_infrastructure):
        ...
        print(
            f"Testing {len(agents_by_tenant)} tenants with {num_actions:,} actions..."
        )

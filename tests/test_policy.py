...
def setup_basic_system():
    ...
    role = RoleDefinition(
        name="reader",
        permissions=[Permission("read_document", "document", same_tenant_only)],
    )
    iam.add_role(role)
...
def test_policy_requires_elevation_for_sensitive_action():
    ...
    sensitive_role = RoleDefinition(
        ...
        permissions=[Permission("approve_payment", "payment", any_condition)],
    )
...
def test_policy_email_domain_allowlist():
    ...
    email_role = RoleDefinition(
        name="emailer",
        permissions=[Permission("send_external_email", "email", any_condition)],
    )
...
def test_permission_condition_returns_false():
    ...
    permission = Permission(
        action="view_record", resource_type="record", conditions=requires_department
    )

    finance_role = RoleDefinition(name="finance_viewer", permissions=[permission])
...
def test_permission_without_conditions():
    ...
    role = RoleDefinition(name="unrestricted_role", permissions=[permission])
    iam.add_role(role)
...
def test_policy_cross_tenant_boundary():
    ...
    role = RoleDefinition(
        ...
        permissions=[Permission("read_data", "data", any_condition)],
    )
    iam.add_role(role)
...
def test_permission_resource_type_mismatch():
    ...
    permission = Permission(
        action="read_data", resource_type="document", conditions=None  # Specific type
    )

    role = RoleDefinition(name="doc_reader", permissions=[permission])
    iam.add_role(role)
